...
def register_features_generator(features_generator_name: str) -> Callable[[FeaturesGenerator], FeaturesGenerator]:
    """
    Creates a decorator which registers a features_mol generator in a global dictionary to enable access by name.

    :param features_generator_name: The name to use to access the features_mol generator.
    :return: A decorator which will add a features_mol generator to the registry using the specified name.
    """
...
def get_features_generator(features_generator_name: str) -> FeaturesGenerator:
    """
    Gets a registered features_mol generator by name.

    :param features_generator_name: The name of the features_mol generator.
    :return: The desired features_mol generator.
    """
    if features_generator_name not in FEATURES_GENERATOR_REGISTRY:
        raise ValueError(f'Features generator "{features_generator_name}" could not be found. '
                         f'If this generator relies on rdkit features_mol, you may need to install descriptastorus.')

    return FEATURES_GENERATOR_REGISTRY[features_generator_name]


def get_available_features_generators() -> List[str]:
    """Returns a list of names of available features_mol generators."""
...
try:
    ...
    def rdkit_2d_features_generator(mol: Molecule) -> np.ndarray:
        ...
        raise ImportError('Failed to import descriptastorus. Please install descriptastorus '
                          '(https://github.com/bp-kelley/descriptastorus) to use RDKit 2D features_mol.')

    @register_features_generator('rdkit_2d_normalized')
    def rdkit_2d_normalized_features_generator(mol: Molecule) -> np.ndarray:
        """Mock implementation raising an ImportError if descriptastorus cannot be imported."""
        raise ImportError('Failed to import descriptastorus. Please install descriptastorus '
                          '(https://github.com/bp-kelley/descriptastorus) to use RDKit 2D normalized features_mol.')


"""
Custom features_mol generator template.

Note: The name you use to register the features_mol generator is the name
...
def custom_features_generator(mol: Molecule) -> np.ndarray:
    ...
    # Replace this with code which generates features_mol from the molecule
    features_mol = np.array([0, 0, 1])
